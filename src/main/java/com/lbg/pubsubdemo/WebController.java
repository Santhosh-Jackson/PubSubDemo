...
public class WebController {
    ...
    @GetMapping("/pull")
    public List<String> pull(@RequestParam("subscription1") String subscriptionName) {

        Collection<AcknowledgeablePubsubMessage> messages =
                this.pubSubTemplate.pull(subscriptionName, 10, true);

        if (messages.isEmpty()) {
            return Collections.singletonList("No messages available for retrieval.");
        }

        try {
            ListenableFuture<Void> ackFuture = this.pubSubTemplate.ack(messages);
            ackFuture.get();
            return messages.stream().map(message -> message.getPubsubMessage().getData().toStringUtf8()).collect(Collectors.toList());

            // return String.format("Pulled and acked %s message(s)", messages.size());
        } catch (Exception ex) {
            LOGGER.warn("Acking failed.", ex);
            return Collections.singletonList("Acking failed");
        }
